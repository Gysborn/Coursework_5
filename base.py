...
class Arena(metaclass=BaseSingleton):
    ...
    def _check_players_hp(self) -> Any:
        ...
        if self.player.hp <= 0 and self.enemy.hp <= 0:
            self.battle_result = "Игра завершилась в ничью"
            self._end_game()
            return self.battle_result
        if self.player.hp <= 0:
            self.battle_result = f"{self.enemy.name} одерживает победу"
            self._end_game()
            return self.battle_result
        if self.enemy.hp <= 0:
            self.battle_result = f"{self.player.name} одерживает победу"
            self._end_game()
            return self.battle_result
        else:
    ...
    def player_hit(self):
        ...
        result_p = self.player.hit(self.enemy)
        result_e = self.next_turn()
        return f"{result_p}\n{result_e}"
    ...
    def player_use_skill(self):
        ...
        # TODO возвращаем результат удара строкой
        result_p = self.player.use_skill(self.enemy)
        result_e = self.next_turn()
        return f"{result_p}\n{result_e}"
